...
func TestGetEmptyUser(t *testing.T) {
	Convey("Retrieve starred repos for empty user", t, func() {
		...
		mstore, err := meta.NewBaseMetaDB(msConfig.MetadataStoreConfig{
			UserMetadataStoreConfig: &msConfig.UserMetadataStoreConfig{
				RootDir: srcConfig.Storage.RootDirectory,
				Driver: "local",
			},
		}, sctlr.Log)
		sctlr.StoreController.MetadataStore = mstore
